...
public final class DataAssetCompanion extends ServiceCompanion {
   ...
   public CompletionStage<DataAccessRequestV2> enrichDataAccessRequest(DataAssetProperties properties, DataAccessRequestProperties request) {
      return projects
         .getProjectById(request.getProject())
         .thenCompose(ProjectEntity::getProperties)
         .thenApply(project -> DataAccessRequestV2.apply(
            request.getId(),
            request.getCreated(),
            properties,
            project,
