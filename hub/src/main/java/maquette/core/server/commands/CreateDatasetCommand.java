...
public class CreateDatasetCommand implements Command {
   ...
   public CompletionStage<CommandResult> run(User user, RuntimeConfiguration runtime, ApplicationServices services) {
      ...
      return services
         .getDatasetServices()
         .createDataset(user, project, dataset, summary, description)
         .thenApply(pid -> MessageResult.apply("Successfully created dataset %s/%s", project, dataset));
   }

   @Override
   public Command example() {
      return CreateDatasetCommand.apply("my-funny-project", "my-funny-dataset", "Lorem ipsum", "Lorem ipsum");
   }
}
