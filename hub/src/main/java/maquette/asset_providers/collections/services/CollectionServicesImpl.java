...
public final class CollectionServicesImpl implements CollectionServices {
   ...
   public CompletionStage<List<String>> listFiles(User executor, String collection) {
      return get(collection).thenCompose(CollectionEntity::list);
   }
   ...
   public CompletionStage<BinaryObject> read(User executor, String collection, String file) {
      return get(collection).thenCompose(c -> c.read(executor, collection, file));
   }
   ...
   private CompletionStage<CollectionEntity> get(String collection) {
      return entities
         .getByName(collection, Collections.TYPE_NAME)
