...
public final class ObjectValidator<T> implements Validator<T> {
   ...
   private final List<Pair<Function3<ValidationContext, String, T, Boolean>, Procedure4<ValidationContext, String, T, Exception>>> checks;

   private final boolean required;

   public static <T> ObjectValidator.Builder<T> build() {
      return ObjectValidator.Builder.apply(Lists.newArrayList());
   }
   ...
   public static class Builder<T> {
      ...
      public <R> Builder<T> validate(String fieldName, Function<T, R> value, Validator<R> validator) {
         var pair = Pair.<Function3<ValidationContext, String, T, Boolean>, Procedure4<ValidationContext, String, T, Exception>>apply(
            (ctx, prefix, obj) -> validator.validate(ctx, String.format("%s.%s", prefix, fieldName), value.apply(obj)),
            (ctx, prefix, obj, ex) -> validator.onException(ctx, String.format("%s.%s", prefix, fieldName), value.apply(obj), ex));
