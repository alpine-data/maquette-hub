...
public final class MlflowClient {
    ...
    public static MlflowClient apply(MlflowConfiguration mlflowConfiguration, UID project) {
        return apply(mlflowConfiguration, project, DefaultObjectMapperFactory
            .apply()
            .createJsonMapper());
    }
    ...
    public List<ModelFromRegistry> getModels() {
        return Optional
            ...
            .map(model -> {
                var versions = query(
                    String.format("/api/2.0/preview/mlflow/model-versions/search?filter=name%%3D%%27%s%%27",
                        model.getName()),
                    ...
                    .map(version -> {
                        ...
                        var commit = run
                            ...
                            .filter(tag -> tag
                                .getKey()
                                .equals("mlflow.source.git.commit"))
                        ...
                        var modelPath = version
                            ...
                            .substring(version
                                .getSource()
                                .indexOf("artifacts") + "artifacts/".length());
