...
public final class DataAssetServicesCompanion extends ServicesCompanion {
   ...
   public <T> CompletionStage<Optional<T>> filterPermission(User user, String name, Function<DataAssetPermissions, Boolean> check, T passThrough) {
      ...
      return Operators.compose(accessRequestsCS, membersCS, workspacesCS, (accessRequests, members, workspaces) -> {
         ...
         accessRequests = accessRequests
            .stream()
            .filter(request -> workspaces.stream().anyMatch(ws -> ws.getId().equals(request.getWorkspace().getId())))
   ...
   /**
    * Returns the pass through if the user is member of the origin workspace and the workspace is allowed to access the data.
    *
    ...
    */
   public <T> CompletionStage<Optional<T>> filterSubscribedConsumer(User user, String name, UID workspace, T passThrough) {
