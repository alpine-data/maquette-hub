...
public final class DatabaseServicesImpl implements DatabaseServices {
        ...
        if (executor instanceof AuthenticatedUser) {
            return users
                ...
                .thenCompose(authToken -> assets
                    .getByName(database)
                    .thenCompose(asset -> databases.analyze(
                        asset,
                        authToken
                            .getId()
                            .getValue(),
    ...
    public CompletionStage<Records> executeCustomQuery(User executor, String database, String query) {
        return assets
            ...
            .thenCompose(settings -> databases.executeCustomQuery(settings, query));
    }
    ...
    public CompletionStage<DatabaseSettings> getDatabaseSettings(User executor, String database) {
        return assets
            ...
            .thenCompose(entity -> entity.getCustomSettings(DatabaseSettings.class))
            .thenApply(settings -> {
                if (!settings.isAllowLocalSession()) {
                    throw LocalSessionsNotAllowedException.apply();
                } else {
                    return settings;
                }
            });
